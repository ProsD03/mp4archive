...
parser.add_argument("-o", "--output", help="desired path for the output")
...
if args.encode:
    if not os.path.isfile(args.encode):
        ...
        exit(1)
    if not args.output:
        sys.stderr.write(f"error: -o/--output must be set to a valid file path for encoding operations.")
        exit(1)
